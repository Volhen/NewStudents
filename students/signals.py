...
def log_group_updated_added_event(sender, **kwargs):
    ...
    if kwargs['created']:
        logger.info("Group added: %s %s (ID: %d)", group.title, group.leader.first_name,group.id)
    else:
        logger.info("Group updated: %s %s (ID: %d)", group.title, group.leader.first_name,group.id)
...
def log_group_deleted_event(sender, **kwargs):
    ...
    group = kwargs['instance']
    logger.info("Group deleted: %s %s(ID: %d)",group.title, group.leader.first_name,group.id)
